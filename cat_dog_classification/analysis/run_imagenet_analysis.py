...
def parse_args():
    ...
    parser.add_argument('--device', type=int, nargs='+', default=[0], help="GPU device index or -1 for CPU")
    
    parser.add_argument('--checkpoint', type=str, default='/raid2/bazaarz/continuous-thought-machines/logs/cat_dog/checkpoint.pt', help="Path to ATM checkpoint")
    parser.add_argument('--output_dir', type=str, default='tasks/cat_dog_classification/analysis/outputs/imagenet_viz', help="Directory for visualization outputs")
    ...
    parser.add_argument('--inference_iterations', type=int, default=25, help="Iterations to use during inference.")
...
if __name__=='__main__':
    ...
    validation_dataset = CDDataset('/raid2/bazaarz/continuous-thought-machines/data/cat_dog/test', transform=transform)
    validation_dataset_centercrop = CDDataset('/raid2/bazaarz/continuous-thought-machines/data/cat_dog/test',  transform=transforms.Compose([
            transforms.Resize(img_size),
    ...
    #         transforms.Normalize(mean=dataset_mean, std=dataset_std) # Normalize
    #     ]))
    class_labels = list(['cat','dog']) # Load actual class names
    ...
    if 'videos' in args.actions:
        ...
        for di in data_indices:
            ...
            ground_truth_target = 0
            inputs = validation_dataset.__getitem__(int(di))
