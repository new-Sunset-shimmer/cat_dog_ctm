...
from data.custom_datasets import CDDataset
...
def parse_args():
    ...
    # torchrun --standalone --nnodes=1 --nproc_per_node=4 -m tasks.cat_dog_classification.train_distributed --log_dir logs/cat_dog  
    # Model Selection
    ...
    parser.add_argument('--positional_embedding_type', type=str, default='learnable-fourier', help='Type of positional embedding (CTM, LSTM).',
                        choices=['none',
                                 'learnable-fourier',
    ...
    parser.add_argument('--n_synch_out', type=int, default=256, help='Number of neurons to use for output synch (CTM only).')
    parser.add_argument('--n_synch_action', type=int, default=256, help='Number of neurons to use for observation/action synch (CTM only).')
    ...
    parser.add_argument('--batch_size_test', type=int, default=64, help='Batch size for testing.')
    parser.add_argument('--lr', type=float, default=1e-3, help='Learning rate for the model.')
    parser.add_argument('--training_iterations', type=int, default=10000, help='Number of training iterations.')
    ...
    parser.add_argument('--num_workers_train', type=int, default=4, help='Num workers training.')
    ...
    parser.add_argument('--device', type=int, nargs='+', default=[0,1,2,3], help='List of GPU(s) to use. Set to -1 to use CPU.')
...
def get_dataset(dataset, root):
    ...
    train_transform = transforms.Compose([
        transforms.RandomResizedCrop(224), #resize
                transforms.RandomHorizontalFlip(), # Flip for data augmentation
                transforms.ToTensor(), # change to tensor
                normalize]) # then normalize RGB value
    ...
    test_transform = transforms.Compose([
        ...
        transforms.CenterCrop(224),
                transforms.ToTensor(),
                normalize])
    full_dataset = datasets.ImageFolder(root='/raid2/bazaarz/continuous-thought-machines/data/cat_dog/train')
    class_ind = {cls: [] for cls in full_dataset.classes}
    for idx, (_, label) in enumerate(full_dataset.imgs):
        classname = full_dataset.classes[label]
        class_ind[classname].append(idx)
    train_indices, val_indices = [], []
    val_frac = 0.2
    rng = np.random.default_rng(seed=42)
    for cls, idxs in class_ind.items():
        idx = np.array(idxs)
        rng.shuffle(idxs)
    ...
    val = Subset(full_dataset, val_indices)
    train_data = CDDataset(which_split=train, transform=train_transform)
    test_data = CDDataset(which_split=val, transform=test_transform)
...
if __name__=='__main__':
    ...
    if args.do_compile:
        # what is this ?
        # compile is new method for speed up
        print('Compiling...')
    ...
    # Training
    # what iter function do ?
    # get iter object from file
